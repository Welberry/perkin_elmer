program pe_dark_stats

  !! Reports some stats on a Perkin Elmer dark current tif file
  ...
  use pnm_class, only: new, pnm_object, write
  ...
  integer :: nx, ny, npixels, nunique, error, ivalue, i, j, maskvalue
  ...
  logical, allocatable :: datamask(:,:), zeromask(:,:)
  ...
  ! Allocate the space required for the data
  allocate(data(nx,ny),datamask(nx,ny),zeromask(nx,ny))
  ...
  do while (have_args())
     ...
     datamask = .false.
     datamask(1:128,1:1024) = .true.
     do j = 1,2
        do i = 1, 16
           datamask = cshift(datamask,shift=128,dim=1)
           ! write(stdout,fmt,advance='no') mean(data,datamask) ! sum(data)/npixels
           write(stdout,fmt,advance='no') sum(data,mask=datamask)/131072
        end do
        datamask = .false.
        datamask(1:128,1025:2048) = .true.
     end do

     write(stdout,*)
     
     where (.not. datamask) data = 0.

     ! call new(pnm, nint(data), origin='tl')
     ! call new(pnm, nint(data))
     ! call write(pnm, "test.pgm")
