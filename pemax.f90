...
!  use rannum_module, only: rseed, rannum
  ...
  real(rd_kind), dimension(:,:), allocatable :: realdata, dark, totdata
  ...
  allocate(totdata(nx,ny),realdata(nx,ny),data(nx,ny),rawdata(nx*ny),overexposed(nx,ny))
...
!!$  end if
  ...
  do while (have_args())
     ...
     where(realdata > totdata) totdata = realdata
...
contains
  ...
  subroutine usage
    ...
    write(stderr,*)
    write(stderr,*) 'Usage: pemax [OPTIONS] pefile(s)'
    write(stderr,*)
